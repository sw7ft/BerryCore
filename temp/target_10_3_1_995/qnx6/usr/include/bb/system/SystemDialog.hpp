 ...
 *
 * @image html cascades_system_dialog_box.png A system dialog box
 *
 * @details The dialog box displayed is an application-modal window, though it is possible to use a system-modal window by setting
...
class BB_SYSTEM_EXPORT SystemDialog : public QObject
{
    ...
    /*!
     * @brief @c includeRememberMe indicates whether a check box for remembering user selection should be included in the dialog box.
     ...
     * the dialog box is also updated immediately to reflect the changes on the UI. If @c autoUpdateEnabled is @c false, the dialog
...
public:
     ...
     * @details When the custom action button resets, its label is an empty @c QString, therefore, this button is
...
public Q_SLOTS:
     ...
     * @details The result of the request can be retrieved with the @c SystemDialog::result property, or by using the @c finished() signal.
